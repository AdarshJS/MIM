...
class Config():
    ...
    def __init__(self):
        ...
        self.kernel_size = (3, 3) # for glass #(11, 11) for everything else
        ...
        self.glass_intensity_upper_th = 150 # threhold intensties that reflect the range of the glass shadow in the mid map
        self.glass_intensity_lower_th = 90
        self.noise_removal_threshold = 15 #20 #2 # blob size threshold to remove small noise patches
        self.intmap_mid_binary_thresh = 10 #threshold to convert grascale imid intensity map to a binary image
        # self.glass_shadow_cleaned = np.zeros_like(self.roi_curr)
    ...
    def intensity_map_mid_cb(self, data):
        ...
        # int_mid[np.where(int_mid==-128)]=0

        # print("min max of int mid callback :",np.min(int_mid),np.max(int_mid))
        ...
        cv2.imshow("Mid Intensity Map", cv2.resize(self.intensitymap_mid_inflated, (400,400), interpolation = cv2.INTER_AREA))
    ...
    def glass_detect(self):
        ...
        # Consider on the ROI of the thresholded mid map and the raw low map
        intmap_mid_thresholded_roi = intmap_mid_thresholded[int(center_x-roi_side):int(center_x+roi_side), int(center_y-roi_side):int(center_y+roi_side)] #masked_intmap2[int(center_x-roi_side):int(center_x+roi_side), int(center_y-roi_side):int(center_y+roi_side)]
        intmap_low_roi = self.intensitymap_low[int(center_x-roi_side):int(center_x+roi_side), int(center_y-roi_side):int(center_y+roi_side)] #masked_intmap2[int(center_x-roi_side):int(center_x+roi_side), int(center_y-roi_side):int(center_y+roi_side)]

        # dilate the ROIs
        kernel = np.ones(self.kernel_size,np.uint8)
        intmap_mid_thresholded_roi = cv2.dilate(intmap_mid_thresholded_roi, kernel, iterations =1)
        intmap_low_roi = cv2.dilate(intmap_low_roi, kernel, iterations =1)
        ...
        # Covert the isolated glass shadow ROI to a binary image        
        ...
        contours, hierarchy = cv2.findContours(glass_shadow_roi_bw, cv2.RETR_EXTERNAL, cv2.CHAIN_APPROX_NONE)
        for contour in contours:
          area = cv2.contourArea(contour)
          if area > self.noise_removal_threshold:
            cv2.fillPoly(glass_shadow_cleaned , [contour], 255)

        kernel = np.ones((1,1),np.uint8)
        # glass_shadow_cleaned= cv2.dilate(intmap_low_roi, kernel, iterations =1)

        # Visualization of the noisy and cleaned ROIs that isolates glass shadow
        cv2.imshow("Glass Blob in ROI", cv2.resize(glass_shadow_roi_bw, (400,400), interpolation = cv2.INTER_AREA))
        cv2.imshow("Noise removed Glass Blob in ROI", cv2.resize(glass_shadow_cleaned , (400,400), interpolation = cv2.INTER_AREA))
        ...
        if(self.flag == 0):
            print("---------inside flag 0-------------")
            # roi_prev = np.zeros(self.roi_shape, dtype=np.uint8)
            self.x_odom_prev = self.x
            self.y_odom_prev = self.y
            self.theta_prev = self.th
            diff1 = self.intensitymap_mid - self.intensitymap_low

            self.diff_curr[int(center_x-roi_side):int(center_x+roi_side), int(center_y-roi_side):int(center_y+roi_side)] = glass_shadow_cleaned + self.diff_curr[int(center_x-roi_side):int(center_x+roi_side), int(center_y-roi_side):int(center_y+roi_side)]
            

            self.diff_curr = diff1
            self.roi_prev = self.diff_curr[int(center_x-roi_side):int(center_x+roi_side), int(center_y-roi_side):int(center_y+roi_side)] ##copy.copy(glass_shadow_cleaned) #
        ...
        # elif(self.flag == 1 and self.counter % 3 == 0 and (abs(self.v_x) > 0.1 or abs(self.v_y) > 0.1)):
            ...
            if self.counter % self.clearing_time == 0:
                self.roi_prev = np.zeros(self.roi_shape, dtype=np.uint8) 
                # diff1 = self.intensitymap_mid - self.intensitymap_low
                # self.diff_curr = diff1
                # self.roi_prev = self.diff_curr[int(center_x-roi_side):int(center_x+roi_side), int(center_y-roi_side):int(center_y+roi_side)] + glass_shadow_cleaned
            ...
            self.diff_curr = diff1
            self.diff_curr[int(center_x-roi_side):int(center_x+roi_side), int(center_y-roi_side):int(center_y+roi_side)] = glass_shadow_cleaned + self.diff_curr[int(center_x-roi_side):int(center_x+roi_side), int(center_y-roi_side):int(center_y+roi_side)]
            # cv2.imshow("Diff curr v1:", cv2.resize(self.diff_curr, (400,400), interpolation = cv2.INTER_AREA))
            self.roi_curr = self.diff_curr[int(center_x-roi_side):int(center_x+roi_side), int(center_y-roi_side):int(center_y+roi_side)] #self.glass_shadow_cleaned 
            ...
            cm_col, cm_row = robot_to_costmap(self, x_rob_prev, y_rob_prev) #prev center wrt the current center
            theta_diff = (self.theta_curr - self.theta_prev)*180/math.pi
            # print(cm_col, cm_row, theta_diff) 

            row_tran = int((roi_side)) - int(cm_row)
            col_tran = int((roi_side)) - int(cm_col)
            rotation = theta_diff
            ...
            # print("row col :",row_tran,col_tran)
            # Pad the ROI prev to add the ROI prev to ROI current with the translation
            if row_tran >=0 and col_tran >=0:
                roi_prev_padded = np.pad(transformed_roi, [(0, row_tran), (0, col_tran)], mode='constant', constant_values=200)
                # print("SHapes of the roi curr and cropped roi prev padded :",np.shape(roi_curr), np.shape(roi_prev_padded[row_tran:(row_tran+roi_l),col_tran:(col_tran+roi_l)]),np.shape(roi_prev_padded))
                self.roi_combined = self.roi_curr + roi_prev_padded[row_tran:(row_tran+roi_l),col_tran:(col_tran+roi_l)]
            elif row_tran >=0 and col_tran <0:
                roi_prev_padded = np.pad(transformed_roi, [(0, row_tran), (abs(col_tran),0)], mode='constant', constant_values=200)
                # print("SHapes of the roi curr and cropped roi prev padded :",np.shape(roi_curr), np.shape(roi_prev_padded[row_tran:(row_tran+roi_l),0:roi_l]),np.shape(roi_prev_padded))
                self.roi_combined = self.roi_curr + roi_prev_padded[row_tran:(row_tran+roi_l),0:roi_l]
            elif row_tran < 0 and col_tran >= 0:
                roi_prev_padded = np.pad(transformed_roi, [(abs(row_tran), 0), (0, col_tran)], mode='constant', constant_values=200)
                # print("SHapes of the roi curr and cropped roi prev padded :",np.shape(roi_curr), np.shape(roi_prev_padded[0:roi_l,col_tran:(col_tran+roi_l)]),np.shape(roi_prev_padded))
                self.roi_combined = self.roi_curr + roi_prev_padded[0:roi_l,col_tran:(col_tran+roi_l)]
            elif row_tran < 0 and col_tran < 0:
                roi_prev_padded = np.pad(transformed_roi, [(abs(row_tran), 0), (abs(col_tran), 0)], mode='constant', constant_values=200)
                # print("SHapes of the roi curr and cropped roi prev padded :",np.shape(roi_curr), np.shape(roi_prev_padded[0:roi_l,0:roi_l]),np.shape(roi_prev_padded))
                self.roi_combined = self.roi_curr + roi_prev_padded[0:roi_l,0:roi_l]

            # cv2.imshow("ROI prev padded :", cv2.resize(roi_prev_padded, (400,400), interpolation = cv2.INTER_AREA))
            # cv2.imshow("ROI prev v2 :", cv2.resize(self.roi_prev, (400,400), interpolation = cv2.INTER_AREA))
            # cv2.imshow("Transformed ROI :", cv2.resize(transformed_roi, (400,400), interpolation = cv2.INTER_AREA))
            # cv2.imshow("ROI curr :", cv2.resize(self.roi_curr, (400,400), interpolation = cv2.INTER_AREA))
            # cv2.imshow("ROI combined:", cv2.resize(self.roi_combined, (400,400), interpolation = cv2.INTER_AREA))

            # self.diff_curr[int(int(cm_row)-roi_side):int(int(cm_row)+roi_side), int(int(cm_col-1)-roi_side):int(int(cm_col-1)+roi_side)] = self.diff_curr[int(int(cm_row)-roi_side):int(int(cm_row)+roi_side), int(int(cm_col-1)-roi_side):int(int(cm_col-1)+roi_side)] + self.roi_combined
            self.diff_curr[int(int(cm_row)-roi_side):int(int(cm_row)+roi_side), int(int(cm_col)-roi_side):int(int(cm_col)+roi_side)] = self.diff_curr[int(int(cm_row)-roi_side):int(int(cm_row)+roi_side), int(int(cm_col)-roi_side):int(int(cm_col)+roi_side)] + transformed_roi 


            print(cm_row, cm_col)
            print(int(int(cm_row)-roi_side), int(int(cm_col)-roi_side))
            ...
            # cv2.imshow("Updated Low Map:", cv2.resize(self.intmap_rgb, (400,400), interpolation = cv2.INTER_AREA))
